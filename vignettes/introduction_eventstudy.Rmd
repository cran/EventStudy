...
You get a **free** API key from our website [EventStudyTools](https://eventstudytools.com/api-key). In the first step we need to authenticate to the web API. There are two ways to handle this two parameters:
...
This API package is designed to perform all analysises we provide on our website. 
Furthermore, all parameters can be set. You are able to set every parameter 
in R (we will provide more details later), or you can perform a fast Event Study 
with the defaults.
...
All results will be written by default into the directory `./results`. You can easily 
change this path by setting it as a parameter:
...
It will write all result files into the result directory. Furthermore,
results will be parsed into a R object. 
...
2. Graphics: In this Vignette we show you how to generate nice (static or interactive) graphs (coming soon)
3. Data Preparation: In this Vignette we show you how to get and prepare data for your Event Study from R (coming soon)
4. RStudio Addins: We wrote a RStudio addin for performing different types of Event Studies
